    ...
    def update(
        ...
        if self._initialized:
            still_waiting = (
                ...
                and global_elapsed_seconds <= self.trajectory.start_time_seconds
            )
            still_arrived = (
                self.arrived and global_elapsed_seconds >= self.trajectory.end_time_seconds
            )
        ...
        if not self.has_started:
            self.current_lat, self.current_lon, self.current_altitude_ft = (
                self.trajectory.get_first_point_state()
            )
            self.current_ground_speed_kmh = 0.0
            self.current_vertical_rate_ft_per_min = 0.0
        elif arrived_now:
            self.current_lat, self.current_lon, self.current_altitude_ft = (
                self.trajectory.get_last_point_state()
            )
            self.current_ground_speed_kmh = 0.0
            self.current_vertical_rate_ft_per_min = 0.0
        else:
